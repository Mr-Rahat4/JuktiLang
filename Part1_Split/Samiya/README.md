# JuktiLang Compiler — Part 1 (Review 1 snapshot)
...
## Status: Review 1 target met

- Lexer tokenizes correctly (UTF-8 Bangla keywords + ASCII identifiers/numbers)
- Parser handles declarations, assignment, `যদি`/`নাহলে`, `দেখাও`, and reports syntax errors
...
- `যতক্ষণ` (while) is finalized in the grammar and already implemented in the parser and codegen internals — intentionally not dispatched yet; see `docs/report.md` Section 2.5. It ships active in `Part2/`.
...
├── examples/       # runnable Part-1-scope sample programs
